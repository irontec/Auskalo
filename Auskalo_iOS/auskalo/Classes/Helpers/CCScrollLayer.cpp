...
#include "/Applications/cocos2d-x-2.1.5/cocos2dx/touch_dispatcher/CCTouchHandler.h"
...
namespace cocos2d
{
	...
	bool CCScrollLayer::initWithLayers(CCArray* layers, int widthOffset)
	{
		...
		m_fMarginOffset = 0;//CCDirector::sharedDirector()->getVisibleSize().width;
	...
	{
		float pageFloat = - getPosition().x / (getContentSize().width - m_fPagesWidthOffset);
	...
	void CCScrollLayer::ccTouchMoved(CCTouch* pTouch, CCEvent* pEvent)
	{
		...
		if (m_iState == kCCScrollLayerStateSliding)
		{
			float desiredX = (m_uCurrentScreen * -1.f * (getContentSize().width - m_fPagesWidthOffset)) + touchPoint.x - m_fStartSwipe;
