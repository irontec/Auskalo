...
void GalderaScene::newGaldera(CCNode* sender)
{
    ...
    switch (correctPosition) {
        ...
        case 3:
            ...
            pErantzuna1->setTitleForState(CCString::create(currentGaldera->erantzuna3.c_str()), CCControlStateHighlighted);
            pErantzuna1->setTag(0);
...
CCSprite* GalderaScene::checkAnswer(CCObject *pSender)
{
    ...
    CCLog("QUESTIONPOINTS: %f", (kQuestionPoints * bonus));
    CCLog("PUNTUAZIOA: %d", kalifikazioa);
    CCLog("BARRACOUNT: %d", barraCount);
    ...
    SimpleAudioEngine::sharedEngine()->playEffect(std::string(CCFileUtils::sharedFileUtils()->fullPathForFilename(sound.c_str())).c_str());
    
    CCLog("Kalifikazioa = %d, galderaCount = %d", kalifikazioa, galderaCount);
