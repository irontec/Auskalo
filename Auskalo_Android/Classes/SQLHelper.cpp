...
void SQLHelper::init()
{
    ...
    _dbPath = fileUtils->getWriteablePath();
...
CCArray* SQLHelper::queryMultzoak(int idKategoria)
{
    ...
    if (idKategoria == -1) {
        sqlite3_prepare_v2(_db, "SELECT * FROM Multzoa INNER JOIN Erabiltzailea_Multzoa ON Erabiltzailea_Multzoa.idMultzoa = Multzoa.idMultzoa GROUP BY idMultzoa", -1, &stmt, NULL);
    } else {
        sqlite3_prepare_v2(_db, "SELECT * FROM Multzoa INNER JOIN Erabiltzailea_Multzoa ON Erabiltzailea_Multzoa.idMultzoa = Multzoa.idMultzoa  WHERE idKategoria = ? GROUP BY idMultzoa", -1, &stmt, NULL);
