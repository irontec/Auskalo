...
bool LoginScene::init()
{
    ...
    CCControlButton *pButton = CCControlButton::create(pLabel, sprite);
    pButton->addTargetWithActionForControlEvents(this, cccontrol_selector(LoginScene::loginCallback), CCControlEventTouchUpInside);
    pButton->setPreferredSize(CCSizeMake(213, 56));
    
    /*CCRect r = CCRectMake(pCloseItem->getPosition().x, pCloseItem->getPosition().y, pCloseItem->getContentSize().width, pCloseItem->getContentSize().height);*/
    
    
    pButton->setPosition(ccp(origin.x + visibleSize.width/2, pEmailTextField->getPosition().y - 56 - 15));
